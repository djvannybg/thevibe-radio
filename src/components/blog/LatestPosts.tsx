...
import PostsSlider, { PostItem } from "./PostsSlider"
...
// Детерминиран формат за SSR и клиент
...
export default async function LatestPosts({ limit = 6, className = "" }: Props) {
  ...
  const items: PostItem[] = posts.map((p) => {
    ...
    return {
      ...
      title: p.title,
      image: p.image ?? null,
      createdAtISO: iso,                          // ISO snapshot
      createdAtText: fmtBG.format(new Date(iso)), // стабилен текст
